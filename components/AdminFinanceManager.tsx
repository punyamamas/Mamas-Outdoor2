...
const AdminFinanceManager: React.FC = () => {
  ...
  const handleEndShift = async () => {
      ...
      // LOGIKA REKAPITULASI:
      // Uang yang disetor (Withdrawal) = Total Uang Fisik - Modal Awal
      // Asumsi: Modal awal ditinggal di laci.
      // Jika uang fisik < modal awal, withdrawal = 0 (karena rugi/kurang)
...
for all using (true) with check (true);`;
  ...
  const currentShiftSystemExpected = activeShift ? (activeShift.start_cash || 0) + summary.cashIn - summary.cashOut : 0;
  const estimatedDiff = endCashPhysical - currentShiftSystemExpected;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden min-h-[600px] flex flex-col">
      ...
      <div className="p-6 flex-1 overflow-y-auto">
        ...
        {/* SHIFT CONTROL SECTION (ALWAYS VISIBLE) */}
        <div className="mb-8">
            {isShiftLoading ? (
                                ...
                                <form onSubmit={handleManualSubmit} className="bg-gray-50 p-4 rounded-xl border border-gray-200 animate-slide-in-right">
                                    ...
                                    <div className="flex flex-col md:flex-row gap-3 items-end">
                                        ...
                                        <div className="flex gap-2">
                                            <button type="submit" className="bg-nature-600 text-white p-2 rounded-lg hover:bg-nature-700"><Save size={18} /></button>
                                            <button type="button" onClick={() => setIsManualEntryOpen(false)} className="bg-gray-200 text-gray-600 p-2 rounded-lg hover:bg-gray-300"><Plus size={18} className="rotate-45" /></button>
                                        </div>
                                    </div>
                                </form>
                            )}
                ...
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                        <h4 className="font-bold text-gray-800 flex items-center gap-2">
                            <Clock size={18}/> Riwayat Shift & Rekapitulasi
                        </h4>
                    ...
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {shiftHistory.length === 0 ? (
                                <tr><td colSpan={7} className="p-8 text-center text-gray-400">Belum ada riwayat shift.</td></tr>
                            ) : (
      ...
      {/* END SHIFT MODAL - UPDATED WITH REKAPITULASI */}
